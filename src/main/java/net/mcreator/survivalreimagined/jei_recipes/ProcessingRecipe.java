...
public class ProcessingRecipe implements Recipe<RecipeInput> {
	...
	public ProcessingRecipe(ItemStack output, NonNullList<Ingredient> recipeItems) {
		this.output = output;
	...
	public static class Serializer implements RecipeSerializer<ProcessingRecipe> {
		...
		private static final MapCodec<ProcessingRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems)).apply(builder, ProcessingRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, ProcessingRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);
		...
		private static ProcessingRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new ProcessingRecipe(ItemStack.STREAM_CODEC.decode(buf), inputs);
		}
