...
public class ForgingRecipe implements Recipe<RecipeInput> {
	...
	public ForgingRecipe(ItemStack output, NonNullList<Ingredient> recipeItems) {
		this.output = output;
	...
	public static class Serializer implements RecipeSerializer<ForgingRecipe> {
		...
		private static final MapCodec<ForgingRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems)).apply(builder, ForgingRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, ForgingRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);
		...
		private static ForgingRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new ForgingRecipe(ItemStack.STREAM_CODEC.decode(buf), inputs);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, ForgingRecipe recipe) {
