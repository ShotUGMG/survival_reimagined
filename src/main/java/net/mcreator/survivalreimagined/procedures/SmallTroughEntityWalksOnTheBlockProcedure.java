...
public class SmallTroughEntityWalksOnTheBlockProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		...
		if (entity.getData(SurvivalReimaginedModVariables.PLAYER_VARIABLES).HungryProc == 1 && entity.getData(SurvivalReimaginedModVariables.PLAYER_VARIABLES).Hungry == true) {
			if ((getPropertyByName(blockstate, "food_portions") instanceof IntegerProperty _getip1 ? blockstate.getValue(_getip1) : -1) > 0) {
				{
					int _value = (getPropertyByName(blockstate, "food_portions") instanceof IntegerProperty _getip3 ? blockstate.getValue(_getip3) : -1) - 1;
