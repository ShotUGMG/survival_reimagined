...
_(Mod uses .toml file, recommended to use a mod to access toml files such as Configured. config file name is srconfig.toml)_

## Compat
   - Integrated compatibility with Dynamic Light's mods (Dynamic Lights, Lamb's Dynamic Lights)

_Holding a torch (or any reasonable light source) with a Dynamic Lights mod installed will now prevent paranoia_

   - Added JEI information tab to all Rune Items

   - Updated Create compat.

_Added recipes for Andesite Alloy, modified recipes for Brass Ingot. If Create Mod is installed Zinc will be renamed to Sphalerite_
...
- Blood Moon
   ...
   _Crimson Thread is now needed to craft Heart of the Bloodmoon_
